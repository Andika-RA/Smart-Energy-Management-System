...
def generate_id(prefix, current_time, sequence):
...
def generate_datasets(num_records=6000):
    ...
    for i in range(num_records):
        ...
        record_id = generate_id("PWR", current_time, i + 1)
        ...
        power_data.append([record_id, timestamp_str, hour, day_of_week, temperature, prev_demand, zone, max(10, power_demand)])
    ...
    for i in range(num_records):
        current_time = start_time + timedelta(hours=i)
        record_id = generate_id("GRD", current_time, i + 1)
        timestamp_str = current_time.isoformat() + "Z"
        ...
        if voltage < 190 or voltage > 245 or power_factor < 0.80:
            ...
            status = "Normal"
        grid_data.append([record_id, timestamp_str, voltage, current, power_factor, temperature, humidity, status])
    df_grid = pd.DataFrame(grid_data, columns=['id', 'timestamp', 'voltage', 'current', 'power_factor', 'temperature', 'humidity', 'grid_status'])
    ...
    for i in range(num_records):
        current_time = start_time + timedelta(hours=i)
        record_id = generate_id("SNS", current_time, i + 1)
        timestamp_str = current_time.isoformat() + "Z"
        ...
        sensor_data.append([record_id, timestamp_str, sensor_value, timestamp_hour, rolling_mean_1h, z_score])
        
    df_sensors = pd.DataFrame(sensor_data, columns=['id', 'timestamp', 'sensor_value', 'timestamp_hour', 'rolling_mean_1h', 'z_score'])
